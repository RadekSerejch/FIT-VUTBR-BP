...
function DetectorDetail({show, closeFunction, detector, points}){
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            tmpTableData.push({direction: detector.properties.FirstDirection_Name, cyclists: 0, pedestrians: 0, avgCyclists: 0, avgPedestrians: 0, avgCyclistsDays: 0, avgPedestriansDays: 0})
            tmpTableData.push({direction: detector.properties.SecondDirection_Name, cyclists: 0, pedestrians: 0, avgCyclists: 0, avgPedestrians: 0, avgCyclistsDays: 0, avgPedestriansDays: 0})
            ...
            jsonResult.features.forEach((element) => {
                    ...
                    tmpHis.push({time: element.properties.EndOfInterval, cyclists: element.properties.FirstDirection_Cyclists + element.properties.SecondDirection_Cyclists, pedestrians: element.properties.FirstDirection_Pedestrians + element.properties.SecondDirection_Pedestrians})
            ...
            setHistoryChartFiltered(tmpHis);
            setIsLoading(false);
        }
    ...
    const filterData = (dateStart, dateEnd) => {
        ...
        history.forEach((element, index) => {
            if(moment(element.properties.EndOfInterval, 'DD.MM.YYYY HH:mm').toDate() >= moment(dateStart, 'YYYY-MM-DD').toDate() && moment(element.properties.EndOfInterval, 'DD.MM.YYYY HH:mm').toDate() <= moment(dateEnd, 'YYYY-MM-DD').toDate() &&
                selectedTimes[moment(element.properties.EndOfInterval, 'DD.MM.YYYY HH:mm').hour()]  && selectedDays[moment(element.properties.EndOfInterval, 'DD.MM.YYYY HH:mm').isoWeekday() - 1]){
                tmpTableData[0].cyclists += element.properties.FirstDirection_Cyclists;
                tmpTableData[1].cyclists += element.properties.SecondDirection_Cyclists;
                tmpTableData[0].pedestrians += element.properties.FirstDirection_Pedestrians;
                tmpTableData[1].pedestrians += element.properties.SecondDirection_Pedestrians;
    ...
    useEffect(() => {
        ...
        filterData(selectedDateFirst, selectedDateSecond);
    ...
    return(
        ...
        <div id="detail">
            ...
            <DateFilter showFilter={showFilter} isLoading={isLoading} minDate={minDate} maxDate={maxDate} filterData={filterData} setSelectedDaysOuter={setSelectedDays} setSelectedTimesOuter={setSelectedTimes} selectedTimesIn={selectedTimes} selectedDaysIn={selectedDays}></DateFilter>
